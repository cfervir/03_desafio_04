...
let lasViviendas = (arrayViviendas = vivienda) => {

  let datosViviendas = '';
  document.getElementById("resultadoBuscador").textContent = `${arrayViviendas.length}`;

  for (let viviendas of arrayViviendas) {
    datosViviendas += `
...
const buscarViviendas = () => {
  ...
  if ((isNaN(numeroCuartos) || numeroCuartos <= 0) || (isNaN(desdeM2) || desdeM2 <= 0) || (isNaN(hastaM2) || hastaM2 <= 0)) {
    ...
    const contenedorViviendas = vivienda.filter(viviendaUnica => (
      viviendaUnica.cuartos === numeroCuartos && viviendaUnica.metros >= desdeM2 && viviendaUnica.metros <= hastaM2
    ));
    
    lasViviendas(contenedorViviendas);
